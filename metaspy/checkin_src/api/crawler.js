...
module.exports = async (cookies, setting) => {
    ...
    if (setting.yearRange || setting.monthRange || setting.dayRange) {
        ...
        if (filteredResult.length > 0) {
            ...
            const filteredFileName = `checkin_data_${hours}${minutes}${day}${month}${year}.json`;
            const filteredOutputPath = path.join(__dirname, '..', 'user_data', filteredFileName);
            if (!fs.existsSync(path.join(__dirname, '..', 'user_data'))) {
                fs.mkdirSync(path.join(__dirname, '..', 'user_data'), { recursive: true });
            }
            fs.writeFileSync(filteredOutputPath, JSON.stringify(filteredResult, null, 2));
            console.log(`Filtered data saved to: ${filteredOutputPath}`);
        }
