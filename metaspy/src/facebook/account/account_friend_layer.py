...
class AccountFriendLayer:
    def __init__(self, user_id: str, max_layers: int, friends_per_node_per_layer: List[int]):
        self.user_id = user_id
        self.max_layers = max_layers
        self.friends_per_node_per_layer = friends_per_node_per_layer
        self.timestamp = datetime.now().strftime("%H%M%d%m%Y")
        self.base_dir = os.path.join("user_data", f"friends_data_{self.timestamp}") 
        self.tree_data = {}
    ...
    def _save_friends_data(self, data: List[Dict[str, Any]], directory: str, filename: str) -> None:
        ...
        account_info = {
            ...
            "layer_info": {
                ...
                "friends_per_node": self.friends_per_node_per_layer[self.current_layer]
            }
        }
    ...
    def _crawl_layer(self, user_id: str, layer: int) -> List[Dict[str, Any]]:
        ...
        max_friends = self.friends_per_node_per_layer[layer]
    ...
    def _process_user_and_friends(self, user_id: str, layer: int, parent_path=None) -> Dict[str, Any]:
        ...
        for idx, friend in enumerate(user_data[:self.friends_per_node_per_layer[layer]]):
    ...
    def crawl(self) -> None:
        ...
        rprint(f"   - Số bạn bè mỗi node ở từng tầng: {self.friends_per_node_per_layer}")
        ...
        result = {
            ...
            "friends_per_node_per_layer": self.friends_per_node_per_layer,
            "crawled_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        ...
        for layer in range(self.max_layers + 1):
            if layer == 0:
                nodes_in_layer = self.friends_per_node_per_layer[0]
                rprint(f"   - Tầng {layer} (bạn bè trực tiếp): {nodes_in_layer} node")
            else:
                nodes_in_layer = num_nodes[layer - 1] * self.friends_per_node_per_layer[layer]
                rprint(f"   - Tầng {layer}: {num_nodes[layer - 1]} x {self.friends_per_node_per_layer[layer]} = {nodes_in_layer} node")
